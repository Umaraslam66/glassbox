...
## What the Overview (front page) shows

The page it opens on. One table, six rows, one per gate: the gate's name, its
verdict, one sentence on what happened, and which sidebar page holds the numbers
behind it.

The verdict in each row is read straight out of that gate's own report file —
`full_sweep_v2_qa.json`, `stage2_v2_recovery.json`, `stage3_gate3.json`,
`stage4_gate4.json`, `stage5_strategies.json` — so the front page cannot disagree
with the runs below it. Gate 0 is the one exception: it was a setup checklist
signed off in conversation, has no report file, and its PASS is written down in
the page.

Under the table, a one-line tally (how many gates graded, how many passed) and,
on a fresh clone, a note listing which report files are not there yet. A gate
with no report reads "no report yet" rather than passing by default.
